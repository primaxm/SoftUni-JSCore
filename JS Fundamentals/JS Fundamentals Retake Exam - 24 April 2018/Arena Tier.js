function arenaTier(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i] === "Ave Cesar") {
            if (!gladiators.isEmpty) printResult(gladiators);
            break;
        } else {
                ...
                if (gladiators.hasOwnProperty(gladiatorOne) && gladiators.hasOwnProperty(gladiatorTwo)) {
                    for (const gladiatorsKey in gladiators[gladiatorOne]) {
                        if (gladiators[gladiatorTwo].hasOwnProperty(gladiatorsKey)) {
                            ...
                            sumOfSkillsOne = Object.keys(gladiators[gladiatorOne])
                                .map(a => gladiators[gladiatorOne][a])
                            ...
                            } else if (sumOfSkillsOne < sumOfSkillsTwo) {
    ...
    function printResult (ladiators) {
...
arenaTier(["Pesho -> PowerPunch -> 300",
    "Pesho -> BattleCry -> 400",
    "Atanas -> BattleCry -> 400",
    "Atanas -> PowerPunch -> 400",
    "Pesho vs Atanas", "Ave Cesar"])